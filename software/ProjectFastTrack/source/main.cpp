	...
	void generateVallidTrackes(){
		...
		for(int i = 0; !lines[i].isEmpty(); i++){
			for(int j = i+1; !lines[j].isEmpty(); j++){
				...
				trackes[center].left = lines[i].centerIndex;
				trackes[center].right = lines[j].centerIndex;
				trackes[center].width = width;
			}
		}
	}
...
void cameraAlgorythmus_2(TrackAnalyse* analysMethod){
	...
	if(analysMethod->lines[0].isEmpty()) {
		...
		for(int i = 1; i < 317; i++){
			uint16_t index = 158 + (i/2) * (i%2==0?-1:1);
			if(!analysMethod->trackes[index].isEmpty()){
				center = index;
				break;
			}
		}
...
int main(){
	...
	Sceduler::getTaskHandle(&cameraAlgorythmus,100);
